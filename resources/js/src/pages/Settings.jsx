            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
              <Card className="border-indigo-100 bg-indigo-50/30 dark:border-indigo-900/30 dark:bg-indigo-900/10">
                <CardHeader>
                  <CardTitle className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    <span>Current Plan: <span className="text-indigo-600 dark:text-indigo-400 font-extrabold">{currentPlan?.name || 'Free'}</span></span>
                    <Badge variant="success" className="bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400 border-none">
                      {formatMoney(currentPlan?.amount_minor, currentPlan?.currency)} / {currentPlan?.interval || 'month'}
                    </Badge>
                  </CardTitle>
                  <CardDescription>
                    {subscription
                ...
                <CardContent className="space-y-4">
                  <div>
                    <div className="flex justify-between text-sm mb-2">
                      <span className="font-medium text-slate-700 dark:text-slate-300">Monthly Email Capacity</span>
                    ...
                    <div className="w-full bg-white dark:bg-slate-950 rounded-full h-2.5 overflow-hidden border border-slate-200 dark:border-slate-800">
                      <div className="bg-indigo-600 h-2.5 rounded-full" style={{ width: subscription ? '100%' : '12%' }}></div>
                    </div>
                  </div>
              ...
              <div className="grid gap-4 lg:grid-cols-3">
                {billingPlans.map((plan) => {
                  ...
                  return (
                    <Card key={plan.key} className={`border ${isCurrent ? 'border-indigo-300 shadow-lg shadow-indigo-100/40 dark:border-indigo-500/40 dark:shadow-none' : 'border-slate-200 dark:border-slate-800'}`}>
                      <CardHeader>
                      ...
                      <CardContent className="space-y-4">
                        <div>
                          <p className="text-3xl font-black text-slate-900 dark:text-slate-50">{formatMoney(plan.amount_minor, plan.currency)}</p>
                          <p className="text-sm text-slate-500">per {plan.interval}</p>
                        </div>
                        <div className="text-sm font-medium text-slate-700 dark:text-slate-300">
                          {(plan.emails_limit || 0).toLocaleString('en-IN')} emails included
                        </div>
                      ...
                      <CardFooter>
                        <Button
                          ...
                          isLoading={billingCheckoutLoading}
                          disabled={billingLoading}
                          onClick={() => handlePlanCheckout(plan).catch(() => {})}
                        >
                          {isCurrent ? 'Update Plan' : ctaLabel}
                        </Button>
                      </CardFooter>
                    </Card>
                  );
